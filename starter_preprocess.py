...
class TextPreprocessor:
    ...
    def fetch_from_url(self, url: str) -> str:
        ...
        """
        # 1. Validate: Check if the URL ends with .txt
        if not url.lower().endswith(".txt"):
            raise ValueError("URL must point to a .txt file")
        try:
            # Fetch: Download the content from the URL
            response = requests.get(url)
            # Check: Raise an exception for HTTP errors (like 404)
            response.raise_for_status()
            return response.text
        except requests.exceptions.RequestException as e:
            raise Exception(f"Failed to fetch text from URL: {e}")
